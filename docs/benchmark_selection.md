...
PARSEC 3.0은 blackscholes(연산/금융), canneal(라우팅 최적화와 불규칙
메모리), dedup(데이터 처리 파이프라인), streamcluster(스트림
클러스터링)를 썼습니다. 네 개 모두 `native.runconf` 인자를 수정하지
않았고 스레드는 네 개입니다.
...
임의 워크로드로 바꾼 것이 아니라 공식 애플리케이션 벤치마크 설정입니다.
