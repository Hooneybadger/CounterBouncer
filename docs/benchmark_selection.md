...
`native.runconf` 인자는 그대로 뒀습니다.

native 입력은 cirosantilli/parsec-benchmark가 미러한 원본 아카이브입니다.
csail-csg 포크는 RISC-V 교차컴파일이 중심이라, x86-64는 Ubuntu에서
돌아가는 쪽을 썼습니다.
...
pipeline check로만 실행했습니다. 이 결과로 앱 성능을 말하진
않습니다.
...
워크로드로 바꾼 게 아니라 공식 애플리케이션 벤치마크 설정입니다.
