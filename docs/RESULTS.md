...
이걸 제출 기능으로 올리지 않은 이유도 측정이 정했다. 효과가 인스턴스 의존적이고(prob_38 −10.6%, prob_27 −2.9%, prob_40 0%), 코어 CP 시간이 들쭉날쭉하며(prob_40 코어 2.8초 vs prob_27 코어 90초+), 무엇보다 CP(ortools)는 *로컬 분석 도구*라 제출 패키지에 실을 수 없다(서버 보장 없음·15MB 초과). 즉 relax-and-repair는 **천장이 schedule-coupling임을 증명한 연구 결과**이지 아직 제출 레버가 아니다. 자기완결(ortools 없는) 코어 스케줄러는 같은 코어-난이도 변동(prob_27 90초) 때문에 비용 대비 이득이 modest해 보류한다. 남은 진짜 도약은 nesting과 스케줄을 *동시에* 푸는 자기완결 전역 최적화인데, 그건 [B5](./STRATEGY_PLAN.md)의 다각형-MIP 금지선과 정면으로 만난다.
