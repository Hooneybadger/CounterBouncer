...
래스터 엔진 생성)만 직접 하고, **구성·ALNS·선호 개선·최종 검증은 전부 fork 자식이 한다.** 메인은
`return_cap = 0.90 × timelimit`까지 큐에서 결과를 거두기만 하다가, 그 시각이 되면 무슨 일이 있어도
가장 좋은 feasible 결과(없으면 floor)를 반환한다. 자식 하나가 느린 Shapely 호출에 묶이거나 OOM으로
죽어도 메인의 벽시계는 자식과 무관하게 흐르므로, 메인은 항상 제한시간 안에 답을 낸다. 이 구조의 한
줄 목적은 *어떤 제한시간에서도 −1(시간 초과·미반환)을 구조적으로 불가능하게* 만드는 것이다.
...
네 제한시간(10·60·300·1800초) 게이트에서 한 번도 infeasible이 없었다. 그래서 P3는 우리가 학습셋에서
본 적 없는 실패 모드를 건드린 것이었다.

정적 분석과 실측으로 모든 infeasible 경로를 좁혔다. improve 경로는 `check_feasibility`로 자체 검증하니
...
반복과 예산 없는 최종 `check_feasibility`도 메인을 묶는다. 느린 서버나 무거운 기하에서 이 tail이 누적돼
...
최고 base를 얻고 시드만 ALNS에서 갈린다** — 이는 v1.0.0의 포트폴리오(최고 base에 4시드)와 정확히 같은
탐색이다. v1.0.0은 이 구성을 메인에서 한 번만 돌려 fork로 공유했지만, 그 무거운 구성이 짧은 제한시간에
메인을 묶어 overrun했다. 이제 구성은 각 자식이 독립으로 돌린다 — 결정적이라 품질은 동일하고, 전용 4코어에선
...
자식이 만드는 `committed`는 각자 고유라 격리된다. 자식 내부 예산은 ALNS 0.83·polish 0.88로, 그 뒤 최종
...
불변식은 *메인에는 deadline을 넘길 수 있는 무거운 일이 없다*는 것 — floor와 ir만 직접 하고 나머지는 종료
가능한 자식 안에 있다. 그리고 floor 자체도 블록별 try로 감싸 *어떤 블록이 망가져도 빈 operations(= 미배치
...
`construct`와 `_improve` *내부*에서 나므로, 단계 사이 가드는 이미 늦은 시점에만 발화해 막지 못했다. 무거운
...
검토했다. daemon 자식은 또 fork할 수 없고, 비-daemon 손자를 쓰면 메인이 중간 자식을 종료할 때 손자가
고아로 남아 제한시간 뒤까지 떠도는 위험이 있었다. 재현이 불가능한 상황에서 이 복잡도는 새 실패 모드를
부를 수 있어 버렸다. 대신 자식마다 독립 구성을 택했는데, 메모리 측정에서 자식당 RSS가 약 35MB라 4개를
...
더해져 더 좋을 것이라 기대했으나, 전체 40개 측정에서 순위 시뮬레이션이 **73 → 58로 퇴보**했다 —
...
base를 얻어 정확히 v1.0.0의 탐색을 복제한다. *고르지 않은 길을 측정으로 확인한* 사례다.
...
cap(`return_cap = 0.93×timelimit`)이 단일 시간 경계이고, 그 뒤 best_sol(있으면)·floor 중 하나를 즉시
반환한다 — 둘 다 이미 만들어진 feasible dict이라 시간 가드로 더 좋은 best를 floor로 강등하지 않는다.
...
사실상 동일하고, 32/40이 동률-이상이다. 순위 시뮬레이션은 head-to-head 72:75로 3점 뒤지지만, 뒤진 8개는
대부분 +0.1~1.3%이고 눈에 띄는 prob_20조차 supervisor 런들에서 149603·156697·163435로 ±5% 출렁이는
ALNS 벽시계 변동이라, 체계적 퇴보가 아니라 노이즈 범위다. (4) **메모리** — 자식당 RSS ~35MB로 OOM 없음.
...
floor를 반환한다 — 품질은 낮지만 feasible이라 −1은 아니다(feasibility-first의 본령). 실제 평가는 게이트
