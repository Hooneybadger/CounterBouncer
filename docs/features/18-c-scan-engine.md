...
한계는 정직하게 적는다. **마샬(래스터화)이 새 병목**이다 — 900블록 18.8초 중 11.8초가 Python `ir.masks()`(Shapely)다. C 스캔(7초)은 더 줄여도 마샬이 남아, 더 큰 도약은 래스터화도 C로 옮기거나 캐시하는 것이다(미측정, 다음 후보). **서버에서 정적 바이너리가 실제로 도는지는 제출로만 확정**되나, 정적·제너릭 빌드 + 폴백으로 리스크를 −1이 아닌 *이득 없음*으로 묶었다. 그리고 이 엔진은 scan 구성기를 *빠르게* 할 뿐 *더 좋게* 하진 않는다 — 339M 아래는 여전히 [features/14 relax](./14-relax-repair.md)·localized repair의 몫이고, 300·1800초의 헤드룸이 거기 있다.
