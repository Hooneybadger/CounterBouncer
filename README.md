...
“측정을 믿을 수 있는가?”는 다른 문제입니다.** Profiler가 아닙니다.
병목도 찾아 주지 않습니다.

## 무엇을 푸는 문제인가

카운터가 찍힌 것과 그 숫자를 믿어도 되는 건 다른 이야기입니다.
평소 속도로 끝났다고 coverage가 충분한 것도 아니고, coverage가
좋아도 실행 환경이 깨끗한 것도 아닙니다.

판정은 두 축입니다. 한 줄 `ACCEPT` / `DEGRADED` / `REJECT`는 두 축을
합친 값입니다.
...
| INVALID | (any) | 진단에 쓰지 않음 |

## 결과

`native-holdout-v2`는 i9-14900K bare-metal에서 PARSEC native 네 개와
CloudSuite Data Caching 270회입니다. warmup 별도, 실행 실패 0.
integrity는 VALID 160, INVALID 110입니다. 한 줄 판정은 ACCEPT 70,
DEGRADED 90, REJECT 110입니다. `native-holdout-v1` 225회는 그대로입니다.
...
INVALID는 값이 틀렸다는 뜻이 아닙니다. MULTIPLEX는
...
CloudSuite는 서버 호스트 PID를 잽니다. pin 안 `cpu-migrations`만
있다고 integrity가 떨어지는 건 아닙니다. 조건은 CLEAN, MULTIPLEX,
SMT, MEMORY, PCORE, HYBRID입니다. REFERENCE는
`{cpu_core/cycles, cpu_core/instructions}` minimal group입니다.
절대 참값은 아닙니다.
