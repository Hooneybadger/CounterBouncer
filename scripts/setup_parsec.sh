...
for item in parsec-3.0-core.tar.gz parsec-3.0-input-native.tar.gz.{0..4}; do
  curl --fail --location --retry 3 --continue-at - "$base/$item" -o "vendor/downloads/$item"
done
...
for package in blackscholes canneal dedup streamcluster; do
  (cd vendor/parsec && bin/parsecmgmt -a build -p "$package" -c gcc -n 4) > "artifacts/setup/parsec-build-$package.log" 2>&1
done
