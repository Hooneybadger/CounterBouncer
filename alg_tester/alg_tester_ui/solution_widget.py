...
from ogc2026.alg_tester.alg_tester_ui.bay_layout_widget import (
    BayCanvas, BlockItem, _auto_scale,
    _layer_gray, _SCALE_MIN, _SCALE_MAX, _SCALE_STEP,
)
from ogc2026.alg_tester.alg_tester_ui.gantt_widget import GanttCanvas, _bay_color, _bay_color_dark
from ogc2026.alg_tester.utils import _resolve_layers
...
class SolutionTab(QWidget):
    ...
    def _on_run_done(self, solution: dict):
        ...
        from ogc2026.alg_tester.utils import check_feasibility
    ...
    def _render_bay_state(self, t: int, ops: list[dict], op_idx: int):
        ...
        if cur_kind == "EXIT":
            ...
            if bay_id is not None and bay_id < len(self._bay_canvases):
                ...
                try:
                    from ogc2026.alg_tester.utils import Bay as UtilsBay, Block as UtilsBlock, check_exit
            ...
            if a is not None:
                ...
                if bay_id < len(self._bay_canvases):
                    ...
                    try:
                        from ogc2026.alg_tester.utils import Bay as UtilsBay, Block as UtilsBlock, check_entry
                        bay_data = self._instance["bays"][bay_id]
                        util_bay = UtilsBay(width=int(bay_data["width"]), height=int(bay_data["height"]))
                        util_present = [
                            UtilsBlock(block_id=bid2,
